...
2. 粒子系统、点云数据
3. new Cesium.CompositeMaterialProperty()，暂时还没搞明白
```js
// 创建CompositeMaterialProperty
const combinedMaterial = new Cesium.CompositeMaterialProperty();
...
// 注意：这里需要根据Cesium版本查看具体的API方式
// 以下是一种可能的正确用法
combinedMaterial.intervals = new Cesium.TimeIntervalCollection([
    new Cesium.TimeInterval({
        start: new Cesium.JulianDate(0),
        stop: new Cesium.JulianDate(1),
        data: stripeMaterial
    }),
    new Cesium.TimeInterval({
        start: new Cesium.JulianDate(1),
        stop: new Cesium.JulianDate(2),
        data: gridMaterial
    })
]);
```
