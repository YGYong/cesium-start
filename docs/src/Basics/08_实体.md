...
- Cesium 中实体很重要
- Cesium 中实体很重要
- 实体内容非常丰富,部分只展示基础案例,`后续会将更多案例补充到实践中`
...
1. 模型动画,模拟飞机不断飞行,使用`viewer.trackedEntity`使相机位置跟随实体
...
| 属性 | 类型 | 默认值 | 描述 |
| --- | --- | --- | --- |
1 | show | Boolean | true | 是否显示椭圆 |
1 | semiMajorAxis | Number | 1.0 | 椭圆的长半轴长度 |
1 | semiMinorAxis | Number | 1.0 | 椭圆的短半轴长度 |
1 | height | Number | 0.0 | 椭圆的高度（相对于地形） |
1 | heightReference | HeightReference | NONE | 高度参考模式：<br>• NONE - 绝对高度<br>• CLAMP_TO_GROUND - 贴地<br>• RELATIVE_TO_GROUND - 相对地面 |
1 | material | MaterialProperty | Color.WHITE | 椭圆的材质（颜色、纹理、特效等） |
1 | granularity | Number | Cesium.Math.RADIANS_PER_DEGREE | 椭圆的粒度（用于简化椭圆） |
1 | outline | Boolean | false | 是否显示轮廓 |
1 | outlineColor | Color | Color.BLACK | 轮廓的颜色 |
1 | outlineWidth | Number | 1.0 | 轮廓的宽度（像素） |
1 | fill | Boolean | true | 是否填充椭圆 |
1 | rotation | Number | 0.0 | 椭圆的旋转角度（弧度） |
1 | stRotation | Number | 0.0 | 纹理旋转角度（弧度） |
1 | extrudedHeight | Number | 0.0 | 椭圆的拉伸高度（相对于地形） |
1 | shadows | ShadowMode | ShadowMode.DISABLED | 指定椭圆是投射还是接收来自光源的阴影 |
1 | distanceDisplayCondition | DistanceDisplayCondition | null | 基于距离的显隐控制 |
