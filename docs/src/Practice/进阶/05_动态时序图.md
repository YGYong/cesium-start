...
该案例源码来自[Cesium-Examples](https://github.com/jiawanlong/Cesium-Examples)，素材在源码`/src/Basics/timeMap`目录下。也可从`Cesium-Examples`中获取。

## 介绍

1. 使用`Cesium.SingleTileImageryProvider`加载单张图片作为时序图
2. 使用`Cesium.ImageryLayer`将时序图添加到`viewer.imageryLayers`中
3. 通过定时器`setInterval`实现动态切换时序图
4. 注意在加载天地图后再初始化时序图，否则天地图会覆盖我们的时序图

<video controls width="600">
  <source src="../../Aassets/Practice/动态时序图.mp4" type="video/mp4" />
  您的浏览器不支持HTML5视频标签。
</video>
...
<script setup>
import Demo from '@/Components/进阶/05_动态时序图.vue'
</script> -->
