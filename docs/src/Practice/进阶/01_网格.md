...
1. 使用`Cesium.Rectangle.fromDegrees()`创建初始全球范围网格
2. 通过`viewer.camera.computeViewRectangle()`获取当前视域边界
3. 使用`viewer.camera.moveEnd`监听相机移动结束后自动更新网格坐标，实现视域范围内的精准覆盖
...
<script setup>
import Demo from '@/Components/进阶/01_网格.vue'
</script>
