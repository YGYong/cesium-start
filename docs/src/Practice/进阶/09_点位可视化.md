...
展示了如何在三维地图上展示多个监测点，并实现点击交互查看详情的功能。案例包含了相机自动定位、点位标记、信息弹窗等实用功能。

![效果](../../Aassets/Practice/可视化点位.png)

## 功能

1. 初始化时自动计算所有点位的边界范围，使用 BoundingSphere 算法确保所有点位都在视野内
2. 相机自动定位到能看到所有点位的最佳视角
3. 点击任意点位弹出详情信息窗口，弹窗跟随相机移动自动更新位置，点击地图空白处或关闭按钮可关闭弹窗

## 核心代码

### 1. 使用 Cesium Entity API 创建点位标记：
...
viewer.scene.postRender.addEventListener(() => {
  if (selectedEntity && popupVisible) {
