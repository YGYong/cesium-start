...
1. 设置起始时间和结束时间，飞行时间为 15 秒
2. 定义飞行路径，包括南京、无锡、苏州和上海的坐标
3. 使用`SampledPositionProperty`创建时间动态的路径
4. 创建飞机模型实体，设置其位置为路径，并使用`VelocityOrientationProperty`来计算模型的朝向
5. 设置时间动态，开启动画
6. 通过`viewer.scene.preUpdate`事件实现第一人称视角跟随飞机

<video controls width="600">
  <source src="../../Aassets/Practice/飞行漫游.mp4" type="video/mp4" />
  您的浏览器不支持HTML5视频标签。
</video>

## SampledPositionProperty 类

需先了解一个Cesium API，`SampledPositionProperty` 是 Cesium 中用于描述随时间变化的位置的核心类，特别适合创建精确控制的运动轨迹。
...
<script setup>
import Demo from '@/Components/进阶/02_飞机飞行.vue'
</script> -->
